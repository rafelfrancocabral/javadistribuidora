...
const PRODUCT_SELECT = 'id, codigo, nome, marca, categoria, subcategoria, preco, unidade, descricao, palavraschave, imagens, video, visivel, estoque, isdestaque, ispromocao, precopromocional, created_at, updated_at';
...
async function saveProduct(e) { e.preventDefault(); const btn = document.getElementById('productSubmit'); showUploading(btn, true); try { const id = document.getElementById('productId').value; let r2NotConfigured = false; const uploadedNew = []; if (pendingUploads && pendingUploads.length) { for (const f of pendingUploads) { const buffer = await f.arrayBuffer(); const hash = await hashFileBuffer(buffer); const [main, thumb] = await Promise.all([fileToWebP(f, 1200), fileToWebP(f, 400)]); const uploaded = await uploadToR2(main, thumb, hash); if (!uploaded) { r2NotConfigured = true; continue; } uploadedNew.push(uploaded.main); } pendingUploads = []; } const urlImgs = []; for (let i = 0; i < 5; i++) { const v = document.getElementById('prodImg' + (i + 1)).value.trim(); if (v) urlImgs.push(v); } editingImages = urlImgs.length ? urlImgs : []; if (uploadedNew.length) editingImages = editingImages.concat(uploadedNew).slice(0, 5); const palavraschave = document.getElementById('prodKeywords').value.split(',').map(s => s.trim()).filter(Boolean); const isp = document.getElementById('prodPromocao').checked; const payload = { codigo: document.getElementById('prodCodigo').value.trim() || null, nome: document.getElementById('prodNome').value.trim(), marca: document.getElementById('prodMarca').value.trim(), categoria: document.getElementById('prodCategoria').value, subcategoria: document.getElementById('prodSubcategoria').value || null, preco: parsePrice(document.getElementById('prodPreco').value), unidade: document.getElementById('prodUnidade').value, descricao: document.getElementById('prodDescricao').value, palavraschave, imagens: editingImages, estoque: parseInt(document.getElementById('prodEstoque').value) || 0, isdestaque: document.getElementById('prodDestaque').checked, ispromocao: isp, precopromocional: isp ? parsePrice(document.getElementById('prodPrecoPromo').value) : 0, visivel: true, updated_at: new Date().toISOString() }; let error; if (id) { const r = await db.from(SUPABASE_PRODUCTS_TABLE).update(payload).eq('id', id); error = r.error; } else { const r = await db.from(SUPABASE_PRODUCTS_TABLE).insert(payload); error = r.error; } if (error) throw new Error(error.message); closeProductModal(); await loadProducts(); renderProducts(); toast(r2NotConfigured ? 'Produto salvo, mas imagens não enviadas (R2 ainda não configurado)' : 'Produto salvo com sucesso'); } catch (err) { console.error(err); toast('Erro: ' + err.message, true); } finally { showUploading(btn, false); } }
